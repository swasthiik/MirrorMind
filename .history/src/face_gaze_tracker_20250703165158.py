...
class FaceGazeTracker:
    ...
    def stream_gaze_overlay_live(self):
        ...
        while True:
            ...
            loop_type = "ComparisonLoop" if smile_detected else ("EscapeLoop" if not face_detected else "Normal")
            context = self._guess_context(gaze_data)
            break_suggested = "Yes" if loop_type in ["EscapeLoop", "FreezeLoop"] else "No"

            self._put_overlay_text(frame, loop_type, duration, break_suggested, "DetectedGazePattern", context)
    ...
    def _guess_context(self, gaze_data):
        if not gaze_data:
            return "Unknown"
        dx = [abs(r[0] - l[0]) for r, l in gaze_data]
