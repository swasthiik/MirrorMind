...
class FaceGazeTracker:
    ...
    def stream_gaze_overlay(self, loop_type="Normal", pattern="DetectedGazePattern", context="AutoDetect", break_suggested="No", max_duration=10):
        ...
        while time.time() - start_time < max_duration:
            ...
            if results.multi_face_landmarks:
                ...
                for face_landmarks in results.multi_face_landmarks:
                    try:
                        ...
                        last_eye_positions.append(left_center)

                        self._draw_robo_eye(frame, right_center)
                        self._draw_robo_eye(frame, left_center)

                        if len(last_eye_positions) > 5:
                            deltas = [
                                abs(last_eye_positions[i][0] - last_eye_positions[i - 1][0])
                                for i in range(1, len(last_eye_positions))
                            ]
                            if max(deltas) > 10:
            ...
            if not face_detected:
                inferred_context = "Away"
            elif eyes_detected and eye_movement_detected:
                inferred_context = "Browsing"
            elif eyes_detected and not eye_movement_detected:
                inferred_context = "Reading"
            elif eyes_detected and blink_detected:
                inferred_context = "Writing"
            else:
